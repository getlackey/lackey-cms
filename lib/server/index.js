...
class Server extends Bindable {
            ...
            .then((socketwares) => {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    let port = self._config.get('http.port'),
                        ...
                        socketResource = ((prefix && prefix.length) ? prefix : '') + '/socket.io';
                    ...
                    server = self._express.listen(port, (err) => {
                        ...
                        self._server = server;
                        SCli.log(__MODULE_NAME, '7. Will listen to socket IO on resource ' + socketResource);
                        self._socket = socketIO.listen(server);
                        self._socket.sockets.on('connection', (socket) => {
    ...
    _initModules() {
        ...
        moduleLoader
            ...
            .forEach((module) => {
                current = current
                    .then(() => moduleLoader.loadRoutes(module, _this._express, _this._falcorRoutes));
            });
...
module.exports = (event) => {
    ...
    return configFactory(event.stage)
        .then((config) => {
            if (!config) {
